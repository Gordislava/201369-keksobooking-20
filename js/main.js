...
var removeHiddenStyle = function (elementsArray) {
...
// заполнение всех карточек

var createCards = function (array) {
  var cardsArray = [];
  for (var i = 0; i < array.length; i++) {
    cardsArray.push(fillCard(array[i]));
  }
  return cardsArray;
};
...
var cardsArray = createCards(items);

var renderCard = function (template, siblingNext) {
  cardsArray = createCards(items);
  for (var i = 0; i < cardsArray.length; i++) {
    fragment.appendChild(cardsArray[i]);
    cardsArray[i].style.visibility = 'hidden';
  }
  map.insertBefore(template, siblingNext);
};

renderCard(fragment, siblingMapElement);

// var card = document.querySelectorAll('.map__card');
// var closePopup = document.querySelectorAll('.popup__close');
// console.log(card);

// перенести функцию закрытия по клику внутрь ф-ции, которая отрисовывает конкретную карточку по клику на пин и удалять обработчик после закрытия карты
...
// var onClosePopupClickEsc = function (evt) {
//   if (evt.key === 'Escape') {
//     if (
//       evt.target
//       && evt.target.matches('.popup__close')
...
// map.addEventListener('click', onClosePopupClick);
// map.addEventListener('keydown', onClosePopupClickEsc);
...
var renderedPins = mapPins.querySelectorAll('button[type = button');

console.log(cardsArray);

// сравнивает alt пина и title карточки, если одинаковые - показывает карточку
var someFunction = function (arrayPins, arrayCards) {
  for (var k = 0; k < arrayPins.length; k++) {
    var currentPinAlt = arrayPins[k].querySelector('img').alt;
    for (var j = 0; j < arrayCards; j++) {
      var currentCardTitle = arrayCards[j].querySelector('.popup__title').textContent;
      console.log(currentCardTitle);
    }
    if (currentPinAlt === currentCardTitle) {
      arrayCards[j].style.visibility = 'visible';
      break;
    }
  }
};

// someFunction(pins, cardsArray);

var onRenderPinClick = function (evt) {
  console.log('purpurpur');
  someFunction(renderedPins, cardsArray);
};

for (var i = 0; i < renderedPins.length; i++) {
  renderedPins[i].addEventListener('click', onRenderPinClick);
}
